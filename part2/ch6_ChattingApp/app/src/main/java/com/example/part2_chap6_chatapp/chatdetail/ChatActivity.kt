package com.example.part2_chap6_chatapp.chatdetail
...
import com.example.part2_chap6_chatapp.Key
import com.example.part2_chap6_chatapp.Key.Companion.DB_CHATS
import com.example.part2_chap6_chatapp.Key.Companion.DB_USERS
import com.example.part2_chap6_chatapp.R
import com.example.part2_chap6_chatapp.databinding.ActivityChatdetailBinding
...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        chatAdapter.registerAdapterDataObserver(object : RecyclerView.AdapterDataObserver() {  // 최신 메시지 보이게 레이아웃 아래로
            override fun onItemRangeInserted(positionStart: Int, itemCount: Int) {
                super.onItemRangeInserted(positionStart, itemCount)

                linearLayoutManager.smoothScrollToPosition(  // 1. RecyclerView  2. Status  3. Position
        ...
        Firebase.database.reference.child(Key.DB_CHATS).child(chatRoomId)
            .addChildEventListener(object : ChildEventListener {
                override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                    ...
                    chatAdapter.submitList(chatItemList.toMutableList())  // toMutableList로 선언해줘야 새로운 List가 생겨 데이터가 들어갈 때마다 갱신이 된다.
                }
