...
import com.example.part2ch4lookupgithub.adapter.RepoAdapter
import com.example.part2ch4lookupgithub.databinding.ActivityRepoBinding
...
class RepoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        repoAdapter = RepoAdapter {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(it.htmlUrl))  // 내장 브라우저 열기
        ...
        binding.repoRecyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                val lastVisiblePosition =
                    linearLayoutManager.findLastCompletelyVisibleItemPosition()  // 마지막으로 보여지는 포지션 가져오기
    ...
    private fun listRepo(username: String, page: Int) {
        ...
        githubService.listRepos(username, page).enqueue(object : Callback<List<Repo>> {
            override fun onResponse(call: Call<List<Repo>>, response: Response<List<Repo>>) {
                ...
                hasMore = response.body()?.count() == 30  // 30개면 true, 아니면 false
                repoAdapter.submitList(repoAdapter.currentList + response.body().orEmpty())  // 기존 리스트에 추가하는 방
            }
