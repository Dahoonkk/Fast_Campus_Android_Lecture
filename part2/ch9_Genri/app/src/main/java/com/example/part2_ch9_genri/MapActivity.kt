...
import com.example.part2_ch9_genri.databinding.ActivityMapBinding
...
class MapActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
                ...
                override fun onChildChanged(snapshot: DataSnapshot, previousChildName: String?) {
                    ...
                    if (markerMap[uid] == null) {
                        ...
                        markerMap[uid]?.position =
                            LatLng(person.latitude ?: 0.0, person.longtitude ?: 0.0)
                    }
                        ...
                        googleMap.animateCamera(
                            CameraUpdateFactory.newCameraPosition(
                                CameraPosition.Builder()
                                    .target(LatLng(person.latitude ?: 0.0, person.longtitude ?: 0.0))
                                    .zoom(16.0f)
                                    .build()
                            )
                        )
    ...
    private fun makeNewMarker(person: Person, uid: String): Marker? {
        val marker = googleMap.addMarker(
            MarkerOptions()
                .position(LatLng(person.latitude ?: 0.0, person.longtitude ?: 0.0))
