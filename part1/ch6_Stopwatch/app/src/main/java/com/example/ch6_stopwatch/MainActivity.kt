package com.example.ch6_stopwatch
...
import com.example.ch6_stopwatch.databinding.ActivityMainBinding
import com.example.ch6_stopwatch.databinding.DialogCountdownSettingBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun start() { // 메인스레드가 아니기 때문에 스레드 오류가 남
        timer = timer(initialDelay = 0, period = 100) {
            if(currentCountdownDeciSecond == 0) {
                ...
                // 오류가 안 나게 하도록 하기 위해 runOnUiThread 활용
                runOnUiThread {
            ...
            if(currentDeciSecond == 0 && currentCountdownDeciSecond < 31 && currentCountdownDeciSecond%10 == 0) {
                val toneType = if(currentCountdownDeciSecond == 0) ToneGenerator.TONE_CDMA_HIGH_L else ToneGenerator.TONE_CDMA_ANSWER // 알림음
